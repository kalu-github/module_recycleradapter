...
import lib.kalu.recyclerview.animation.AlphaInAnimation;
import lib.kalu.recyclerview.animation.BaseAnimation;
import lib.kalu.recyclerview.animation.ScaleInAnimation;
import lib.kalu.recyclerview.animation.SlideInBottomAnimation;
import lib.kalu.recyclerview.animation.SlideInLeftAnimation;
import lib.kalu.recyclerview.animation.SlideInRightAnimation;
...
//    private final RecyclerView.RecycledViewPool pool = new RecyclerView.RecycledViewPool();
    ...
    public int getItemCount() {
        if (null == onData() || onData().size() == 0) {
            return getEmptyViewCount() + getHeadViewCount() + getFootViewCount();
        } else {
            return onData().size() + getHeadViewCount() + getFootViewCount();
        }
    }
