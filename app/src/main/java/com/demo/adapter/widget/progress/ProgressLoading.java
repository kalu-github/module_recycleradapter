...
/**
 * description: Loading ProgressBar
 * created by kalu on 2016/10/28 18:15
 */
public class ProgressLoading extends View {

    private final String TAG = ProgressLoading.class.getSimpleName();
    ...
    private void parseAttributes(TypedArray a) {
        ...
        circleRadius =
                (int) a.getDimension(R.styleable.ProgressLoading_pl_circleRadius, circleRadius);

        fillRadius = a.getBoolean(R.styleable.ProgressLoading_pl_fillRadius, false);

        barWidth = (int) a.getDimension(R.styleable.ProgressLoading_pl_barWidth, barWidth);

        rimWidth = (int) a.getDimension(R.styleable.ProgressLoading_pl_rimWidth, rimWidth);

        float baseSpinSpeed =
                a.getFloat(R.styleable.ProgressLoading_pl_spinSpeed, spinSpeed / 360.0f);
        spinSpeed = baseSpinSpeed * 360;

        barSpinCycleTime =
                a.getInt(R.styleable.ProgressLoading_pl_barSpinCycleTime, (int) barSpinCycleTime);

        barColor = a.getColor(R.styleable.ProgressLoading_pl_barColor, barColor);

        rimColor = a.getColor(R.styleable.ProgressLoading_pl_rimColor, rimColor);

        linearProgress = a.getBoolean(R.styleable.ProgressLoading_pl_linearProgress, false);

        if (a.getBoolean(R.styleable.ProgressLoading_pl_indeterminate, false)) {
            spin();
        }
